...
try {
    ...
    // Verify encoding integrity before patching
    if (!content.includes('课程目录')) {
        console.error('ERROR: File appears corrupted/garbled before patching!');
        process.exit(1);
    }
    ...
    const replacements = [
        ...
        // Title & Info
        { r: /font-size: 24px;/g, v: 'font-size: 18px;' }, // Careful with this one, might match others
        ...
        { r: /border-radius: 16px;/g, v: 'border-radius: 10px;' }, // Generic
    ...
    // Restore the missing style scoped tag if needed (from my previous error)
    // Actually git checkout restored it to correct state, so we don't need to fix my bug from before.
    // Unless the git restore brought back the version WITH the bug? No, HEAD should be clean.

    // Double check encoding after replace
    if (!modified.includes('课程目录')) {
        console.error('ERROR: Encoding corrupted DURING patch!');
        process.exit(1);
    }
